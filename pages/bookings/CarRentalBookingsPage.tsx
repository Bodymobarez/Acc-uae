...
interface CarRentalBookingsPageProps {
  serviceTypeFocus: ServiceType.CAR_RENTAL;
}

const CarRentalBookingsPage: React.FC<CarRentalBookingsPageProps> = ({ serviceTypeFocus }) => {
  ...
  const carRentalBookings = useMemo(() => {
    return allBookings.filter(booking => booking.services.some(service => service.serviceType === ServiceType.CAR_RENTAL));
  ...
  const openModalForCreate = () => {
    ...
    const defaultCarRentalService = allServices.find(s => s.name.toLowerCase().includes("car") || s.name.toLowerCase().includes("rental"));
    const initialServiceItem: BookingServiceItem[] = [];
    if (defaultCarRentalService && serviceTypeFocus === ServiceType.CAR_RENTAL) {
        initialServiceItem.push({
            id: generateId(),
            serviceId: defaultCarRentalService.id,
            serviceName: defaultCarRentalService.name,
            serviceType: ServiceType.CAR_RENTAL,
            cost: defaultCarRentalService.defaultCost,
            price: defaultCarRentalService.defaultPrice,
            quantity: 1,
        });
    }
  ...
  return (
    <div>
      <PageHeader title={t('bookings.carRentalBookingsPageTitle')} subtitle={t('bookings.carRentalBookingsPageSubtitle')}>
        <Button onClick={openModalForCreate} leftIcon={<IconPlus />}>
          {t('bookings.addNewCarRentalBooking')}
        </Button>
      </PageHeader>

      <Table<Booking> columns={columns} data={carRentalBookings} renderRowActions={renderRowActions} />
