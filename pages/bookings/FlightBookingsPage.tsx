...
import { Booking, BookingServiceItem, BookingStatus, Client, Employee, TourismService, ServiceType, BookingCategory } from '../../types';
...
const initialBookingFormState: BookingFormState = {
  ...
  bookingCategory: BookingCategory.INTERNATIONAL, 
  services: [],
...
interface FlightBookingsPageProps {
  serviceTypeFocus: ServiceType.FLIGHT;
}

const FlightBookingsPage: React.FC<FlightBookingsPageProps> = ({ serviceTypeFocus }) => {
  ...
  const [selectedServiceTypeToAdd, setSelectedServiceTypeToAdd] = useState<ServiceType>(serviceTypeFocus);

  const flightBookings = useMemo(() => {
    return allBookings.filter(booking => booking.services.some(service => service.serviceType === ServiceType.FLIGHT));
  ...
  const baseServiceOptions: SelectOption[] = allServices.filter(s => s.name.toLowerCase().includes("flight") || s.defaultPrice > 500).map(s => ({ value: s.id, label: s.name })); 
  ...
  const columns = React.useMemo(() => [
    ...
    }, // Corrected: ensures object is closed
  ...
  const addServiceItem = () => {
    ...
    if (serviceDetails) {
      ...
      setSelectedServiceTypeToAdd(serviceTypeFocus);
    }
  };
  ...
  const openModalForCreate = () => {
    ...
    const newFileNumber = `TF-${new Date().getFullYear()}-${String(allBookings.length + 101).padStart(3,'0')}`;
    setBookingForm({...initialBookingFormState, fileNumber: newFileNumber, bookingCategory: BookingCategory.INTERNATIONAL}); // Default to International for Flights
    
    const defaultFlightService = allServices.find(s => s.name.toLowerCase().includes("flight"));
    const initialServiceItem: BookingServiceItem[] = [];
    if (defaultFlightService && serviceTypeFocus === ServiceType.FLIGHT) {
        initialServiceItem.push({
            id: generateId(),
            serviceId: defaultFlightService.id,
            serviceName: defaultFlightService.name,
            serviceType: ServiceType.FLIGHT,
            cost: defaultFlightService.defaultCost,
            price: defaultFlightService.defaultPrice,
            quantity: 1,
        });
    }
    setCurrentServiceItems(initialServiceItem);
    setSelectedServiceTypeToAdd(serviceTypeFocus);
    setIsModalOpen(true);
  };
  ...
  const openModalForEdit = (booking: Booking) => {
    ...
    setSelectedServiceTypeToAdd(serviceTypeFocus); // Keep focus
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingBooking) {
      setAllBookings(allBookings.map(b => b.id === editingBooking.id ? bookingData : b));
    } else {
  ...
  ), [t, handleDeleteBooking, navigate]); // openModalForEdit removed from deps as it's stable

  return (
    <div>
      <PageHeader title={t('bookings.flightBookingsPageTitle')} subtitle={t('bookings.flightBookingsPageSubtitle')}>
        <Button onClick={openModalForCreate} leftIcon={<IconPlus />}>
          {t('bookings.addNewFlightBooking')}
        </Button>
      </PageHeader>

      <Table<Booking> columns={columns} data={flightBookings} renderRowActions={renderRowActions} />

      <Modal isOpen={isModalOpen} onClose={closeModal} title={editingBooking ? t('bookings.editBooking') : t('bookings.createBooking')} size="3xl">
            ...
            <Card className="bg-secondary-50 p-4">
              {currentServiceItems.map((item, index) => (
                <div key={item.id} className="grid grid-cols-12 gap-x-2 gap-y-1 mb-2 items-center border-b border-secondary-200 pb-2">
                  ...
                  <Input wrapperClassName="col-span-6 md:col-span-2 mb-0" label={t('bookings.cost')} type="number" step="0.01" value={item.cost} onChange={(e) => handleServiceItemChange(index, 'cost', e.target.value)} />
                  <Input wrapperClassName="col-span-6 md:col-span-2 mb-0" label={t('bookings.price')} type="number" step="0.01" value={item.price} onChange={(e) => handleServiceItemChange(index, 'price', e.target.value)} />
                  <div className="col-span-12 md:col-span-2 flex items-end justify-end pb-0 md:pb-1 mt-1 md:mt-0">
