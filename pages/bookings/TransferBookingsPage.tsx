...
interface TransferBookingsPageProps {
  serviceTypeFocus: ServiceType.TRANSFER;
}

const TransferBookingsPage: React.FC<TransferBookingsPageProps> = ({ serviceTypeFocus }) => {
  ...
  const transferBookings = useMemo(() => {
    return allBookings.filter(booking => booking.services.some(service => service.serviceType === ServiceType.TRANSFER));
  ...
  const openModalForCreate = () => {
    ...
    const defaultTransferService = allServices.find(s => s.name.toLowerCase().includes("transfer"));
    const initialServiceItem: BookingServiceItem[] = [];
    if (defaultTransferService && serviceTypeFocus === ServiceType.TRANSFER) {
        initialServiceItem.push({
            id: generateId(),
            serviceId: defaultTransferService.id,
            serviceName: defaultTransferService.name,
            serviceType: ServiceType.TRANSFER,
            cost: defaultTransferService.defaultCost,
            price: defaultTransferService.defaultPrice,
            quantity: 1,
        });
    }
  ...
  return (
    <div>
      <PageHeader title={t('bookings.transferBookingsPageTitle')} subtitle={t('bookings.transferBookingsPageSubtitle')}>
        <Button onClick={openModalForCreate} leftIcon={<IconPlus />}>
          {t('bookings.addNewTransferBooking')}
        </Button>
      </PageHeader>

      <Table<Booking> columns={columns} data={transferBookings} renderRowActions={renderRowActions} />
