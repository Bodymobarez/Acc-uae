...
interface VisaBookingsPageProps {
  serviceTypeFocus: ServiceType.VISA;
}

const VisaBookingsPage: React.FC<VisaBookingsPageProps> = ({ serviceTypeFocus }) => {
  ...
  const visaBookings = useMemo(() => {
    return allBookings.filter(booking => booking.services.some(service => service.serviceType === ServiceType.VISA));
  ...
  const baseServiceOptions: SelectOption[] = allServices.filter(s => s.name.toLowerCase().includes("visa")).map(s => ({ value: s.id, label: s.name })); 
  ...
  const openModalForCreate = () => {
    ...
    setBookingForm({...initialBookingFormState, fileNumber: newFileNumber, bookingCategory: BookingCategory.DOMESTIC}); // Default to Domestic for VISA
    
    const defaultVisaService = allServices.find(s => s.name.toLowerCase().includes("visa"));
    const initialServiceItem: BookingServiceItem[] = [];
    if (defaultVisaService && serviceTypeFocus === ServiceType.VISA) {
        initialServiceItem.push({
            id: generateId(),
            serviceId: defaultVisaService.id,
            serviceName: defaultVisaService.name,
            serviceType: ServiceType.VISA,
            cost: defaultVisaService.defaultCost,
            price: defaultVisaService.defaultPrice,
            quantity: 1,
        });
    }
  ...
  ), [t, handleDeleteBooking, navigate]); // openModalForEdit removed from deps

  return (
    <div>
      <PageHeader title={t('bookings.visaBookingsPageTitle')} subtitle={t('bookings.visaBookingsPageSubtitle')}>
        <Button onClick={openModalForCreate} leftIcon={<IconPlus />}>
          {t('bookings.addNewVisa')}
        </Button>
      </PageHeader>

      <Table<Booking> columns={columns} data={visaBookings} renderRowActions={renderRowActions} />
