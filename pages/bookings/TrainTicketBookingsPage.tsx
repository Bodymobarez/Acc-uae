...
interface TrainTicketBookingsPageProps {
  serviceTypeFocus: ServiceType.TRAIN_TICKET;
}

const TrainTicketBookingsPage: React.FC<TrainTicketBookingsPageProps> = ({ serviceTypeFocus }) => {
  ...
  const trainTicketBookings = useMemo(() => {
    return allBookings.filter(booking => booking.services.some(service => service.serviceType === ServiceType.TRAIN_TICKET));
  ...
  const openModalForCreate = () => {
    ...
    const defaultTrainService = allServices.find(s => s.name.toLowerCase().includes("train") || s.name.toLowerCase().includes("ticket"));
    const initialServiceItem: BookingServiceItem[] = [];
    if (defaultTrainService && serviceTypeFocus === ServiceType.TRAIN_TICKET) {
        initialServiceItem.push({
            id: generateId(),
            serviceId: defaultTrainService.id,
            serviceName: defaultTrainService.name,
            serviceType: ServiceType.TRAIN_TICKET,
            cost: defaultTrainService.defaultCost,
            price: defaultTrainService.defaultPrice,
            quantity: 1,
        });
    }
  ...
  return (
    <div>
      <PageHeader title={t('bookings.trainTicketBookingsPageTitle')} subtitle={t('bookings.trainTicketBookingsPageSubtitle')}>
        <Button onClick={openModalForCreate} leftIcon={<IconPlus />}>
          {t('bookings.addNewTrainTicket')}
        </Button>
      </PageHeader>

      <Table<Booking> columns={columns} data={trainTicketBookings} renderRowActions={renderRowActions} />
...
export default TrainTicketBookingsPage;
